...
封装两类输出：
- render_certificate_pdf(data, template) -> BytesIO  单张证书 PDF
- render_batch_zip(project_id, template_id) -> BytesIO  按项目批量打包 ZIP
"""
import os
import zipfile
...
_STYLES = {
    'formal': '该同学在本次志愿服务活动中表现优秀，认真履行职责，圆满完成各项任务，'
              '展现了良好的责任意识与服务精神，特此证明。',
    'warm': '该同学以满腔热情投入志愿服务，用行动温暖他人，用付出诠释担当，'
            '在平凡岗位上展现了不平凡的志愿者风采，特此致谢。',
    'concise': '该同学按要求完成志愿服务任务，服务态度认真，过程表现良好，特此证明。',
}


def _commendation_text(style):
    return _STYLES.get(style, _STYLES['formal'])


# ==================== 单张证书渲染 ====================

def render_certificate_pdf(data, template):
    """根据证书数据 + 模板生成单张 PDF，返回 BytesIO。

    data: dict 来自 certificate_service.get_certificate_data()
    template: CertTemplate 实例
    """
    _ensure_fonts()
    ...
    line1 = f'兹证明  {user_name}  同学（学号：{student_id}）,'
    line2 = f'系  {college}  {major}  专业学生,'
    line3 = f'于  {sign_in_time}  参加了'
    line4 = f'"{project_title}"  志愿服务活动,'
    line5 = f'累计服务时长  {duration}  小时.'
    line6 = _commendation_text(template.commendation_style)
    line7 = '特此证明.'
    ...
    _draw_wrapped(c, line6, x=110, y=y, max_width=width - 220, line_height=22)
    y -= 60

    c.setFont(body_font, 14)
    c.drawString(110, y, line7)
    ...
    c.drawString(sign_x, sign_y + 30, template.signature_text)
...
def render_batch_zip(project_id, template_id=None):
    ...
    if not template:
        raise ValueError('未找到可用证书模板，请先在模板管理中启用至少一个模板')
    ...
    with zipfile.ZipFile(zip_buf, 'w', zipfile.ZIP_DEFLATED) as zf:
        for user_id in eligible:
            try:
                ...
                continue
            pdf_buf = render_certificate_pdf(data, template)
